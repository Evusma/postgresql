...
BEGIN
    ...
    IF (NEW.geom2 IS NOT NULL 
	AND ST_Within(NEW.geom2,(SELECT geom FROM _cadastre_plu_domaine.nsm_domaine_tout WHERE ST_Contains(geom,NEW.geom2))))THEN
		UPDATE _securite.nsm_squats
    ...
    IF (NEW.geom2 IS NULL
        AND NEW.geom IN (SELECT geom FROM _cadastre_plu_domaine.nsm_domaine_tout )) THEN
	    UPDATE _securite.nsm_squats
